{
  description = "Bonsai sync host integration";
  inputs = {
  ...
  outputs = { self, nixpkgs, toolchain, core, axis, sync, datacentre, scenarios, cctv, buildup, wall, pipe, ifc }:
    let
      ...
      make = system:
        let
          ...
          '';
          check = pkgs.runCommand "usdaeco-bonsai-check" {
            nativeBuildInputs = [ kit.pythonEnv kit.usd-dev ];
          } (setup + ''
            cp -R ${self} source
            chmod -R u+w source
            cd source
            env -u PYTHONPATH PYTHONPATH="$AECO_CORE:$PWD" python check.py
            mkdir -p "$out"
          '');
    ...
    in {
      packages = each (system: let p = make system; in { default = p.pkgs.runCommand "usdaeco-bonsai-source" {} ''mkdir -p "$out"; cp -R ${self}/. "$out/"''; });
