...
def frame_camera(stage, paths, size=(1280, 800), *, context=False):
    """Fit the edited bodies or the facility with a 15% margin."""
    ...
    for path in paths:
        ...
        if not body or body.ComputeVisibility() == 'invisible' or body.ComputePurpose() not in ('default', 'render'):
            raise ValueError('Edited body must be visible with purpose default or render')
    ...
    if context:
        bounds.UnionWith(cache.ComputeWorldBound(stage.GetDefaultPrim()).ComputeAlignedRange())
    camera = UsdGeom.Camera(stage.GetPrimAtPath('/Renders/' + ('overview' if context else 'edited_corner')))
    ...
    back = Gf.Vec3d(1, 2, 5).GetNormalized()
    up = Gf.Vec3d(0, 0, 1)
...
def roundtrip(source, model, directory):
    ...
    wall_property = Sdf.Path(selected['wall']['path']).AppendProperty('xformOp:transform')
    session.capture_base([wall_property, wall_property.GetPrimPath().AppendProperty("xformOpOrder")])
    with Usd.EditContext(session.stage, session.intent):
        from pxr import Gf, UsdGeom
        matrix = Gf.Matrix4d(UsdGeom.Xformable(wp).GetLocalTransformation())
        matrix.SetTranslateOnly(matrix.ExtractTranslation() + Gf.Vec3d(0, .15, 0))
        UsdGeom.Xformable(session.stage.GetPrimAtPath(wall_property.GetPrimPath())).MakeMatrixXform().Set(matrix)
    session.intent.Save()
    author(session, selected['pipe']['path'], ['length=' + str(length + .2)])
    # Successful apply consumes intent; retain the requested drivers for review.
    session.intent.Export(str(directory / 'intent.usda'))
    ...
    if result['accepted'] < 2 or result['pending']:
        raise AssertionError({k: v for k,v in result.items() if k != 'diagnostics'} | {'blocking': [d for d in result['diagnostics'] if d['blocking']]})
    exported = Path(session.document('bonsai'))
    ...
    imported_session = host_class('ifc').initialize(str(reimport / 'model.usda'), str(exported), reimport / 'session',kind_import=True)
    ids = [row['ref'] for row in selected.values()]
    print('== stage: compare reimported USD drivers and bodies', flush=True)
    original_host = engine.adapter(session,'bonsai')
    try:
        before = original_host.readback(ids)
    finally:
        original_host.close()
    pipe_receipt = next(row for row in before['touched'] if row['ref'] == selected['pipe']['ref'])
    resolved_length = np.linalg.norm(np.array(pipe_receipt['drivers']['aeco:axis:end']) -
                                     np.array(pipe_receipt['drivers']['aeco:axis:start']))
    if not np.isclose(resolved_length, length + .2, atol=1e-6, rtol=0):
        raise AssertionError('Native pipe did not apply the requested 0.20 m extension')
    ...
    repeat = engine.apply(session,'bonsai')
    if repeat['mutations']:
    ...
    # Publication must use the re-imported, converged view.
    for row in selected.values():
        row['path'] = str(next(p for p in imported.Traverse()
                              if p.GetAttribute('aeco:id') and
                              p.GetAttribute('aeco:id').Get() ==
                              next(r['id'] for r in before['touched'] if r['ref'] == row['ref'])).GetPath())
