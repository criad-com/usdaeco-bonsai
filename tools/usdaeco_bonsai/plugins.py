...
def register_camera_plugins():
    """Validate the pinned camera schema for its importer on the split core."""
    from pxr import Plug, Usd
    register_plugins()
    from usdaeco_cctv import _manifest
    registry = Plug.Registry()
    core = registry.GetPluginWithName("usdAeco")
    camera = registry.GetPluginWithName("usdAecoCctv")
    if core.metadata["aeco"]["tier"] != "core" or "aecoDerived" not in core.metadata.get("SdfMetadata", {}):
        raise RuntimeError("Core must register the derived property metadata")
    if not camera or camera.metadata.get("aeco") != _manifest():
        raise RuntimeError("Rebuild usdAecoCctv from its library manifest")
    if Usd.SchemaRegistry().FindAppliedAPIPrimDefinition("AecoCctvSensorAPI") is None:
        raise RuntimeError("Register camera plugins before creating the schema registry")
    return camera


def import_camera_stage(*args, **kwargs):
    """Keep the released camera import, adapting only its registration callback."""
    from unittest.mock import patch
    register_camera_plugins()
    from usdaeco_cctv import importer
    with patch.object(importer, "register_plugins", register_camera_plugins):
        return importer.import_cctv(*args, **kwargs)
