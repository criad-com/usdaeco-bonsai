from usdaeco_bonsai.runtime import python as run_python
...
from usdaeco_bonsai.gates.compare import compare, close
from usdaeco_bonsai.gates.s4_intent import elements
...
def prepare(directory, case=None):
    ...
    if case:
        from usdaeco_bonsai.gates.wp4 import configure
...
def author_case(session, case):
    ...
    if case.get("relations") or case.get("type") or case.get("typeRaw"):
        from usdaeco_bonsai.gates.wp4 import author_extra
...
def run_suite(directory, hosts=("ifc", "bonsai"), selected=()):
    ...
    for case in cases:
        ...
        if case.get("family") == "datacentre":
            from usdaeco_bonsai.gates.datacentre import prepare as prepare_dc, run_case
        ...
        if case.get("family") == "camera":
            from usdaeco_bonsai.gates.cctv import run_case
            rows = {}
            for host in hosts:
                if host in case["hosts"]:
        ...
        for host in hosts:
            ...
            repeated = cli("--stage", session.path, "apply", "--host", host)
            assert repeated["mutations"] == 0, repeated
            entry["repeatMutations"] = repeated["mutations"]
