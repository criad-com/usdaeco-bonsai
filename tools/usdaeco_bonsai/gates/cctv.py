...
def run_case(directory, case, host="ifc"):
    ...
    assert repeat["mutations"] == 0, repeat
    if refusal:
        assert repeat["pending"] == 1 and digest(session.document(host)) == baseline_hash, repeat
    else:
        assert repeat["inSync"], repeat
    row = dict(id=case, host=host, mutations=result["mutations"], repeatMutations=repeat["mutations"], nativeEvidence=result["nativeEvidence"], passed=True, codes=codes, pending=result["pending"], accepted=result["accepted"])
